...
class Simulator(core.Type):
    ...
    def _loop_update_stimulus(self, step, stimulus):
        ...
        if self.stimulus is not None:
            ...
            stimulus[self.model.cvar, :, :] = self.stimulus(stim_step).reshape((self.model.cvar.shape[0], -1, 1))
            
    def _loop_update_control(self,step,input):
        if self.uinput is not None:
            stim_step = step - (self.current_step + 1)
            uinput = self.gcouple * self.uinput(stim_step)
    ...
    def __call__(self, simulation_length=None, random_state=None):
        ...
        for step in range(self.current_step + 1, self.current_step + n_steps +1):
            ...
            #add control here
            #self._loop_update_control(step,uinput)
            
            #Now we actually go into the integrator; Need to merge in control here somehow
